...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Endpoint para simular progressão automática de status dos pedidos
  app.post("/api/orders/:id/auto-progress", async (req, res) => {
    try {
      ...
      switch (order.status) {
        ...
        case "preparing":
          ...
          break;
        case "ready":
          newStatus = order.fulfillmentMethod === "delivery" ? "shipped" : "completed";
          break;
        case "shipped":
          newStatus = "completed";
          break;
        default:
          return res.json(order); // Já está no status final
      }
  ...
  // Simular progressão automática de todos os pedidos ativos a cada 30 segundos
  setInterval(async () => {
    try {
      ...
      for (const order of activeOrders) {
        // 30% de chance de progredir cada pedido
        if (Math.random() < 0.3) {
          let newStatus = order.status;
          switch (order.status) {
            case "pending":
          ...
          if (newStatus !== order.status) {
            ...
            console.log(`Order ${order.id} progressed from ${order.status} to ${newStatus}`);
          }
        }
      }
    } catch (error) {
